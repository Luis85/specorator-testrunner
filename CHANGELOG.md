...
- The generated `.testrunner` carries a versioned `testrunner-manifest.json`;
  ...
  mismatch — the detection rail Repair uses to regenerate an out-of-date runner.
...
- The generated `.testrunner` now runs Gherkin through **playwright-bdd**
  ...
  tags. Consistent with the beta's reset-not-migrate policy for `data.json`,
  there is **no automatic V1→V2 runner migration**: the playwright-bdd switch is
  a clean break, so a `.testrunner` generated by a pre-playwright-bdd build
  (Test Hub 1.0.x, cucumber-js) is rebuilt from scratch via **Reset Test Hub**
  rather than migrated in place. `Repair installation` regenerates the managed
  files and reinstalls dependencies on a manifest-version change while preserving
  the user's `overwrite:false` steps/pages (ADR-0021, US-051/US-052).
