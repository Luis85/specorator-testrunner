...
**Story Map** _(accepted — see ADR-0027, extended by ADR-0028, ADR-0030)_:
A **sibling overlay to the PRD** (not a node in the Domain → PRD → Use Case tree) that shapes the product journey, vault-local and single-user. Identified as `SM-NNN`, anchored to a product via its `product` field (a PRD id). It holds the facts the single-parent tree was designed not to hold: an audience lane (`users`, resolved **Persona** refs), an ordered **backbone** (`activities`), task-level **steps**, and ordered **release slices** (`slices`, first = the walking skeleton), over rich **Story Map Cards**. Stored one folder per map at `<storyMapsPath>/<id>-<slug>/<id>-<slug>.md`; cards live as their own notes under the map's `cards/` subfolder (ADR-0030). Composes *above* an **Example Map** along the shared `UC-NNN` seam; the two are never conflated. Deliberately not interoperable with, or collaborative like, the storymaps.io tool it parallels (ADR-0028).
...
**Story Map Card** _(rich model — see ADR-0028, ADR-0030)_:
A placement on a **Story Map**, persisted as its own `type: story-map-card` note (`SMC-NNN`) under the map's `cards/` folder (ADR-0030). The note's frontmatter carries the (activity, step, slice) coordinate and map-owned planning attributes — a **card type** (one of five; drives the Legend colour), a free-text **title**, a hand-set **planning status** (`planned`/`in-progress`/`done`/`blocked`, distinct from a Use Case's run-derived automation status), **story points**, **tags**, an optional **color** override, and an optional `UC-NNN` `ref` (a reference-less card is a free-text story not yet promoted to a Use Case). The note's Markdown body is free text (description, acceptance notes) and survives board saves unchanged. Card ids (`SMC-NNN`) are allocated client-side by `addCard` to keep the board's in-memory model in sync with the on-disk state; ids are monotonic per session so a freed id is never reused within the debounce window. A referenced card renders its title plus a resolved, aliased wikilink `[[<note name>\|UC-NNN]]` so titled notes never dangle.
_Avoid_: Ticket, cell. (A reference-less card IS a "story" here, per ADR-0028.)

**Card Type**:
One of five fixed types a **Story Map Card** carries — `task`, `note`, `question`, `edge-case`, `design` — each mapped to a distinct pastel colour (`task`=yellow, `note`=blue, `question`=green, `edge-case`=pink, `design`=purple), matching storymaps.io's Legend. The board renders cards as pastel sticky-notes with dark ink for contrast; a static Legend block shows all five types and colours. An optional per-card `color` field overrides the type colour. The card-type picker is exposed in the **card modal** (`StoryMapCardModal`); inline board edits (title, status, swatch) do not change the type.
_Avoid_: Category, label, tag (tags are a separate freeform field on the card).

**Persona** _(see ADR-0030)_:
A reusable audience archetype stored as a `type: persona` note (`PER-NNN`) in the shared `personasPath` (default `Personas/`). A **Story Map**'s `users` lane holds an ordered list of persona refs; the board renders each as an editable chip in the top audience lane. Personas are shared across maps (not owned by any single map). On map create and on board save, the service materialises a persona note for each user name as a best-effort side-effect — a persona write failure is logged but does not fail the map write. Persona notes carry a Markdown body for goals and context and are queryable via Bases.
_Avoid_: User, audience member, stakeholder (in the domain sense; "user" is acceptable in UI copy for the lane label).

**Story Map Board** _(see ADR-0029)_:
The interactive visual rendering of a **Story Map** in the main workspace view — a users lane, activity/step columns, and slice rows of card tiles. An editable view over the note frontmatter (the single source of truth); the managed Markdown table is kept in sync. P1 shipped the read-only board; P2 added card drag-and-drop (drag a card to another (activity, step, slice) cell); P3 added structure reordering (drag a column/activity or row/slice header); P3b adds creating structure (a `+` inserts a placeholder activity/slice/step) and renaming any of them in place (double-click a header → edit). P3c adds removing structure — a `×` removes an activity/slice (**rejected** when any card references it, so a card is never orphaned), a step (its cards **degrade** to no-step, hanging under the activity), or a card — plus reordering steps (drag a step header onto another of the same activity) and adding a card (a `+ card` per cell inserts a placeholder). P4 adds inline card editing: double-click a card to rename it in place, click its color **swatch** to cycle a palette, and click its **status chip** to cycle the planning status; full attribute editing (points/tags/ref/coordinate) stays in the card modal (which the board picks up as an external update). All board-native edits persist through the same debounced, signature-guarded save (interact.js, pointer-based, behind a swappable adapter). The remaining phase adds zoom/pan + focus mode.
