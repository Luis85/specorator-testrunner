...
The interactive visual rendering of a **Story Map** in the main workspace view — a users lane, activity/step columns, and slice rows of card tiles. An editable view over the note frontmatter (the single source of truth); the managed Markdown table is kept in sync. P1 shipped the read-only board; P2 added card drag-and-drop (drag a card to another (activity, step, slice) cell); P3 added structure reordering (drag a column/activity or row/slice header); P3b adds creating structure (a `+` inserts a placeholder activity/slice/step) and renaming any of them in place (double-click a header → edit). P3c adds removing structure — a `×` removes an activity/slice (**rejected** when any card references it, so a card is never orphaned), a step (its cards **degrade** to no-step, hanging under the activity), or a card — plus reordering steps (drag a step header onto another of the same activity) and adding a card (a `+ card` per cell inserts a placeholder). All persist through the same debounced, signature-guarded save (interact.js, pointer-based, behind a swappable adapter). Later phases add inline card editing + color and zoom/pan.
