...
| **P3-4** | ⏸️ **Deferred (explicitly).** Branded value objects (T5): `type VaultPath = string & { readonly __brand }` with a `vaultPath()` smart constructor running `PathSafetyPolicy`, making construction the chokepoint ADR-0008 claims. (Also closes part of P0-1's root cause.) Tracked, not done in this pass. | 🟡 Medium | DOM-L1, ARCH-L2 |
