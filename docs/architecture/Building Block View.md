...
| `PrdExplorerView` | Workspace leaf | Browse the Domain → PRD → Use Case hierarchy as a tree; launch the builder; open a PRD note. View type `e2e-test-hub-prds`; its body is hosted in the Test Hub **Plan** section (WS-B1). | `prd.created`, `prd.deleted` |
