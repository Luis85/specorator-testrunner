...
| 1.6 | Extract `LiveRefresh` from the five views (review §4). **— delivered 2026-06-16: a shared `LiveDashboardView` base (owns the `LiveRefresh` open/close lifecycle + the empty/header preamble) now backs all seven live views (PRDs, Use Cases, Test Suites, Use Case detail, Evidence Explorer, dashboard, Guided Tour); the duplicated onOpen/onClose/render-preamble clone is gone.** | V2 adds new views (triage, readiness, step library); copy six instead of refactoring eight |
...
| # | Item | Gate |
| --- | --- | --- |
| 3.1 | Spike: generate a playwright-bdd `.testrunner` beside the demo content; validate existing `.feature` compatibility, Cucumber JSON/Messages output through the (now port-based) import pipeline, evidence generation, and cancel/single-run semantics on POSIX **and** Windows **— delivered 2026-06-13 (spike findings: `docs/superpowers/specs/2026-06-13-phase3-1-playwright-bdd-spike-findings.md`; ADR-0021 confirmed).** | Spike findings recorded; ADR from 2.4 confirmed or amended |
| 3.2 | Execute US-051/US-052: swap the generated runner to playwright-bdd with typed step stubs; repair migrates V1 `.testrunner` projects non-destructively (via 2.2) with a clear report; keep cucumber-JSON import as a fallback during the transition window **— delivered 2026-06-13 (PR #44; the import stays cucumber-JSON, and per the pre-announcement beta the migration is a clean-cut with no V1 fallback).** | Demo test green; migrated sample vault green |
| 3.3 | Validation: full unit/integration suite, `e2e-smoke` green on all OSes (via 0.2), the **Guided Tour completes end-to-end against the migrated runner** (its step predicates key on run scopes, spec events, and the demo/`@tour` content — ADR-0020), docs updated (README disclosure, Getting Started, CONTEXT.md terms) **— delivered 2026-06-13: full suite + all-OS `e2e-smoke` green (incl. a Guided-Tour `@tour` e2e leg that runs the self-authored cycle against the real runner) and user-facing docs refreshed. The playwright-bdd migration is complete; §8 V2.0 feature work may begin.** | **Only after this gate does V2.0 feature work (§8) begin** |
