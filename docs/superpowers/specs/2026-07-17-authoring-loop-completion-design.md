...
| D6 | **Coverage cache is content-addressed on BOTH inputs** — `stepTextsHash` (the Feature's Gherkin step texts) *and* `defsHash` (the loaded step-definition pattern set) — instead of #77's sketched `defsRevision` event counter. `allStepsDefined` already loads the patterns for its static pass, so this costs no extra I/O and also catches **external** step-file edits (the counter would miss a definition deleted outside the plugin — the dangerous stale-"covered" direction). |
