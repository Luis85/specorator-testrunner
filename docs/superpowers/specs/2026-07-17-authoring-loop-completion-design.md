...
| D6 | **Coverage cache is content-addressed on BOTH inputs** — `featureHash` over the **Feature file's raw bytes** (step templates alone would miss Scenario Outline Examples-row edits, which change the pickles bddgen evaluates — Codex P2 on PR #102) *and* `sourcesHash` over the **raw step-file sources (path + bytes)** — instead of #77's sketched `defsRevision` event counter. Raw sources, not the scraped pattern set: bddgen coverage can depend on step-file code the scraper cannot model (custom parameter types, regex helpers, variable-built definitions), so a pattern-only digest could serve a stale verdict after such code is edited (Codex P2 on PR #102). **Digest scope = the whole runner source tree.** `sourcesHash` covers every `*.ts` under `.testrunner/src` (steps, pages, support, fixtures), not just `src/steps` — the default generated runner's example step imports `../pages/ExamplePage`, so a `src/steps`-only digest would miss page-object/support edits bddgen compiles (Codex P2 on PR #102). This supersedes the earlier "self-containment gate," which was both too blunt (one default `../pages` import disabled the cache for **every** normal vault) and too porous (its `../` regex missed alias/package/dynamic escapes) — the gate is removed. The static pattern scrape stays scoped to `src/steps`. The digest **also includes the runner-root config files** — `playwright.config.ts` (bddgen's `defineBddConfig`: `features`/`featuresRoot`/`steps`/`tags` globs) **and `tsconfig.json`** (module resolution / `paths` aliases can make bddgen resolve the same step source differently) — so a config edit changes what bddgen evaluates and invalidates; they are the outermost vault-resident bddgen inputs, hashed alongside the `src` tree and feature bytes (Codex P2s on PR #102). After this, the only unhashed inputs are **truly external** — node_modules/package code, or absolute paths outside the runner — exotic for a plugin-generated runner (whose steps import only `playwright-bdd`), session-scoped (D7), managed/regenerated by Repair, and re-recorded on the next detect. **Spawn-window validation (both inputs):** bddgen reads the `.feature` **and** compiles the runner sources from disk mid-spawn, so the verdict is recorded only if a post-spawn re-read still matches **both** the pre-spawn feature bytes and the pre-spawn source set — an external edit to either landing inside the spawn window skips the record (miss-safe); an edit-and-revert entirely within the window is the accepted unobservable residual for both (Codex P2s on PR #102). **Single settings snapshot:** the feature bytes, the source set, and the runner `cwd` bddgen spawns against are all derived from **one** `settingsService.load()` per detect — a second load could digest a newly-edited `testRunnerPath` while bddgen still ran against the old runner, storing a verdict under a runner bddgen never evaluated (Codex P2 on PR #102). **The cache records `covered` only when bddgen's report is UNAMBIGUOUS for this feature; otherwise it abstains (skips the write) and lets the static heuristic decide.** bddgen's `Missing step definitions:` report is not reliably scoped to the one targeted feature (hence the existing `keepFeatureSteps` intersection down to this feature's steps). So neither the raw header nor the filtered list is a correct standalone signal — the raw header can carry *another* feature's misses (false not-covered), and the filtered list can drop an Outline miss whose concrete pickle `I have red` never maps to the `I have <colour>` template (false covered). **POSITIVE-ONLY record (convergence, Codex P2s on PR #102).** The cache stores ONLY a confirmed-covered verdict — never a not-covered one — so `allStepsDefined = cache-confirms-covered OR static-says-all-defined`; a consult can only UPGRADE the static heuristic (surface a covered the static matcher can't model — optional/alternative cucumber-expression syntax), never BLOCK it. The record is one-way: **no missing-steps header → record `covered`**; **any header present → record NOTHING** (this feature falls through to static). This terminates the covered-signal oscillation that churned across ~20 review rounds. bddgen's misses conflate a genuinely-ABSENT definition with a runtime DATA mismatch — an Outline `Given I have <count>` with example `red` against an existing `I have {int}` def, where bddgen reports the concrete `I have red` miss even though the definition plainly exists — and the "Steps" stage cares only about definition EXISTENCE, which the static tier decides (its outline-sentinel makes `<count>` match `{int}`, correctly reading the def as present). Deferring a header to static is therefore the RIGHT answer, not a false-green: the def exists → Steps clears; whether an example row's data matches `{int}` at RUN time is the Run stage's concern, surfaced when the scenario runs. Because the cache can never serve a blocking `false`, a mis-scoped/cross-feature header can no longer pin an actually-covered feature on Steps, and the earlier "record `covered=false` on any header" (which did exactly that) is gone. **Reliable-inputs precondition:** the coverage source snapshot distinguishes a *listing failure* (`.testrunner/src` unreadable — a transient/adapter fault) from a genuinely-empty dir; on a listing failure the record path **abstains** (no verdict written) and the consult path **skips the cache** (falls back to static) rather than digesting an empty snapshot — otherwise a verdict keyed to the empty digest would keep hitting stale while listing stays broken, so source edits would stop invalidating it (Codex P2 on PR #102). Same principle as the ambiguity abstention: **record a verdict only when every input is reliable** — feature readable, sources listed cleanly, bddgen report unambiguous for this feature.

**A listed-but-unreadable coverage-source `.ts` file now ABSTAINS** — `loadRunnerCoverageSources` returns `null` on any per-file read fault, the same as a directory-listing failure (Codex P2 on PR #102), closing the former per-file residual (a partial digest could otherwise keep matching a stale verdict while the omitted file is edited on disk). **Two accepted exotic residuals remain** (documented, not fixed — the common path is already correct, each self-heals on the next clean detect, and positive-only makes each SAFER: a stale hit can now only ever be a wrongly-served `covered`, which the next real detect corrects): (a) the settings-derived `BDD_FEATURES` scoping (from `featureFilesPath`) is not part of the cache key, so a mid-session reconfiguration of `featureFilesPath` after a covered detect (feature bytes + `.testrunner` sources unchanged) could serve the prior verdict until the next detect; (b) the confident-covered branch assumes bddgen actually **evaluated** this feature — "no missing header → covered" is only valid when bddgen generated pickles for it, so a runner where bddgen evaluates *zero* pickles (a user-hard-coded `tags` filter in `playwright.config.ts` excluding the scenarios — the generated template instead reads the `BDD_TAGS` env that `detectMissingSteps` clears) would read `covered=true` for an unevaluated feature. Both require *non-default* runner customization or a rare persisting settings change, are Repair-resettable, and clear on the next clean detect; a robust guard for (b) needs bddgen output-dir introspection. Fixing them has diminishing returns against the safety already guaranteed for the normal path, so they are accepted rather than chased (Codex P2s on PR #102). Two lossy layers are bypassed: the filtered `missingSteps` list (a Scenario Outline's undefined step is reported by bddgen as a concrete pickle like `I have red`, which never maps back to the `I have <colour>` template, so the filtered list can come out empty for a genuinely-undefined outline step — Codex P2), and the snippet parser itself (`parseBddgenMissingSteps` can under-extract if bddgen's snippet formatting shifts, so a header-says-N-but-parsed-zero case would falsely read covered — Codex P2). The header count is bddgen's own authoritative verdict; gating on it keeps the safe direction — any reported miss → not covered (Codex P2s on PR #102). **No post-generate re-detect; rail-only refresh (root fix, product-owner pick after 5 refresh-wiring bugs — Codex P2s on PR #102).** Generate does **not** auto-run a follow-up `detectMissingSteps`: that re-detect (a) published the zero-missing detect that prematurely completed the Guided Tour's implement-steps step (bddgen counts the `throw new Error("Pending")` stubs as *defined*), and (b) forced a rail refresh whose full-view `reload()` cleared `loopResult`/rebuilt rows and kept clobbering inline results (failed/no-op rows, multi-feature mixed errors). Instead, after a generate **or a committed per-row Detect** (both record a coverage-cache verdict that can flip `allStepsDefined` — Validate does not) the detail view does a **rail-only re-derive** — recompute the loop rail's `stepsDefined` fact (`allStepsDefined`) and re-project the rail, leaving `loopResult`, the `FeatureRow`s, and health lines untouched. (Because the re-derive is rail-only it no longer clobbers a Detect's inline result — which is exactly why the detect-event subscription could be dropped; the row-level emit replaces it — Codex P2 on PR #102.) The rail re-derive is **serialized** (through a dedicated `RenderScheduler`) so two overlapping refreshes — e.g. two rows generating close together — coalesce to one trailing derive that reads latest state, and an earlier-computed-but-later-resolving derivation can't overwrite the fresher rail with a stale one (Codex P2 on PR #102). Because it never clears inline state, the rail-only re-derive is harmless and **unconditional**, so (post-Task-8) it is driven by a SEPARATE `useEventBus` binding subscribed to exactly `stepdefinition.generated` and `specification.missingSteps.detected` — the Pending Steps companion's coverage-changing events (WS1/C2). Those two stay OUT of the full-reload `REFRESH_ON` set (a reload would rebuild the `FeatureRow`s and drop an in-flight inline Validate), but the rail-only binding advances the rail off Steps as soon as the companion generates/verifies (spec §3.4). `useEventBus`'s own `RenderScheduler` serializes a burst into one trailing derive; a `railGeneration` counter guards the rail-derive-vs-full-reload race. The cache records a verdict on the **next real detect** — the Pending Steps panel's Verify (Task 7) or a manual detect — not at generate time (the panel's Generate no longer auto-re-detects, which would zero-miss on the pending stubs and prematurely complete the Guided Tour — Codex P2 on PR #102). Consequence: a **common** feature's rail advances instantly after generate (the static tier already sees the written stubs); an **advanced-Gherkin** feature (custom parameter types the static matcher can't model) advances on its next Verify rather than instantly — the accepted cost, consistent with #77's "static is the safe default." Error/no-op rows and the "Generated N stubs" line all survive. |
