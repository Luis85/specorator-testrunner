...
| D6 | **Coverage cache is content-addressed on BOTH inputs** — `featureHash` over the **Feature file's raw bytes** (step templates alone would miss Scenario Outline Examples-row edits, which change the pickles bddgen evaluates — Codex P2 on PR #102) *and* `sourcesHash` over the **raw step-file sources (path + bytes)** — instead of #77's sketched `defsRevision` event counter. Raw sources, not the scraped pattern set: bddgen coverage can depend on step-file code the scraper cannot model (custom parameter types, regex helpers, variable-built definitions), so a pattern-only digest could serve a stale verdict after such code is edited (Codex P2 on PR #102). **Digest scope = the whole runner source tree.** `sourcesHash` covers every `*.ts` under `.testrunner/src` (steps, pages, support, fixtures), not just `src/steps` — the default generated runner's example step imports `../pages/ExamplePage`, so a `src/steps`-only digest would miss page-object/support edits bddgen compiles (Codex P2 on PR #102). This supersedes the earlier "self-containment gate," which was both too blunt (one default `../pages` import disabled the cache for **every** normal vault) and too porous (its `../` regex missed alias/package/dynamic escapes) — the gate is removed. The static pattern scrape stays scoped to `src/steps`. Accepted residual: imports that escape `.testrunner/src` entirely (a tsconfig path alias, a bare package carrying step logic, the playwright config at the runner root) are not hashed — exotic for a plugin-generated runner (whose steps import only `playwright-bdd`), session-scoped (D7), and re-recorded on the next detect. **Spawn-window validation (both inputs):** bddgen reads the `.feature` **and** compiles the runner sources from disk mid-spawn, so the verdict is recorded only if a post-spawn re-read still matches **both** the pre-spawn feature bytes and the pre-spawn source set — an external edit to either landing inside the spawn window skips the record (miss-safe); an edit-and-revert entirely within the window is the accepted unobservable residual for both (Codex P2s on PR #102). **Single settings snapshot:** the feature bytes, the source set, and the runner `cwd` bddgen spawns against are all derived from **one** `settingsService.load()` per detect — a second load could digest a newly-edited `testRunnerPath` while bddgen still ran against the old runner, storing a verdict under a runner bddgen never evaluated (Codex P2 on PR #102). Every generate entry point (detail rows AND the command palette) triggers the best-effort re-detect so a changed step-source set re-records promptly. **Rail refresh is scoped to a coverage-changing generate, not the detect event:** the detail view refreshes its loop rail after a generate outcome **that actually wrote stubs** (`coverageChanged` — the same `generatedSteps.length > 0` signal that gates the post-generate re-detect) — it subscribes to **neither** generation event (`specification.missingSteps.detected` — which also fires for a user's row-local "Detect missing steps", whose awaited reload would unmount the `FeatureRow` and drop its inline detect result — **nor** `stepdefinition.generated`, whose mid-outcome reload would bump the stale-guard *before* the post-generate re-detect records coverage, skipping the intended refresh and stranding the rail on the pre-cache reload — Codex P2s on PR #102). The self-refresh is the sole generation trigger; a palette generate while the view is open is the accepted narrow non-refresh residual. It also does **not** refresh on a *failed* or *no-op* generate, because the refresh clears `loopResult`/rebuilds the row and would wipe an actionable `RUNNER_NOT_INSTALLED` or "nothing to generate" message before the user reads it (Codex P2 on PR #102). A successful generate's own "Generated N stubs" line is intentionally superseded by the refreshed row (forward momentum). |
