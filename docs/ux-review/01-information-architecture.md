...
| R3 | **Single onboarding orchestrator** + docked rail; retire `Get started` panel + tour CTA banner duplication | H | M | Behavior change to first-run; tour event wiring must stay intact | `guided-tour-service` state, `projectTour`; R1 shell to host the rail |
