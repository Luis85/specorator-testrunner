...
- Evidence note ↔ Run ↔ the Suite/Use Case that produced it. **Note (Codex catch): Evidence/Run are NOT id-resolvable like PRD/UC/SM** — Evidence is addressed by `VaultPath` (`TraceabilityRecord.evidence: VaultPath[]`) and `RunHistoryService` has only `list()` (no `findById`). So the nav target is a **discriminated union** spanning all node kinds — `{kind:"artifact", id}` (PRD/UC/SM), **`{kind:"feature", path}`** (Features open by vault path; Scenario References encode `featurePath`), **`{kind:"suite", path}`** (`TestSuite` carries a note path), `{kind:"evidence", path}`, `{kind:"run", runId}` — and "open the run behind this evidence" needs a run/evidence lookup added; it is not free from `openArtifact(id)` alone.

Standardize on **one** navigation port so every node links the same way (today links are ad-hoc `openView` calls with no target). Its target is the **discriminated union** above — `{kind:"artifact", id}` (PRD/UC/SM), `{kind:"feature", path}`, `{kind:"suite", path}`, `{kind:"evidence", path}`, `{kind:"run", runId}` — **not** an id-only `openArtifact(id)` signature (Feature/Suite/Evidence/Run aren't id-resolvable; Codex catch).
...
| R2 | **Deep-link the artifact graph** (PRD→UC, board card→UC, breadcrumb→specific PRD, UC→Story Map board via id; Feature/Suite/Evidence via path; Run via run-id) through a deep-link port with a **discriminated-union target** (artifact-id / feature-path / suite-path / evidence-path / run-id — Codex catch), not id-only | H | M | Stale/renamed-id dead-ends (handle like UC-detail not-found); Feature/Suite/Evidence/Run need path/run-id targets | `findById` for PRD/UC/SM; path targets for Feature/Suite/Evidence; a run/evidence lookup for the Evidence↔Run hop |
