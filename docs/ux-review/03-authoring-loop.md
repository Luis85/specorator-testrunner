...
- **Inline result + evidence:** on terminal, render the result counts and **failed-scenario rows that link straight to the feature** *and* embed an evidence summary inline. **This needs a reusable evidence-note/content projection — `evidence-explorer-rows.ts` does not aggregate it** (reviewer catch, Codex 2026-06-21): that module only projects run-history *table cells* (`runId`, status, counts, scope, date, `evidencePath`) — following it would just duplicate an Evidence Explorer row, not the scenario/artifact evidence body. Add (or reuse) a dedicated evidence-note content projection before scoping inline evidence here. "Open evidence" then becomes "the evidence is already here; open the full note for traces/screenshots."
- **"Re-run failed only"** alongside Re-run (`test-console-view.ts:239`). **Source the failure set from THIS run, not global history** (reviewer catch, Codex 2026-06-21): `ScenarioHistoryService.latestStatuses()` exposes only the **latest global status per Scenario Reference**, so after an *unrelated* older failure elsewhere in the vault it would re-run extra scenarios; the run-specific `runId` lives in the per-run NDJSON with **no read API**. So the workstream must **capture the current run's failed Scenario References** (from the run the console is showing) or add a **`runId`-filtered failure-set query**. **And the run itself is NOT free in the current plumbing**: `ExecutionScope` is only `use-case\|feature\|suite\|all\|demo` (`test-run.ts:10-12`) and `RunLauncher.launch` forwards a single scope/target (`run-launcher.ts:46-50`) — delivering this needs **either a new `ExecutionScope` failure-set (a list of Scenario References) threaded through the runner command, or a generated temporary tag/grep filter**, plus the matching event/evidence handling. Highest-value micro-interaction in the loop, but it carries real runner work — not just a button.

### 3.7 **Light visual identity** (fixes Mi5) — native + a thin BDD skin
Layer a *light, consistent identity* expressed through the brand accent, rhythm, and iconography — **with two clearly separated token families** (per A1):

- **Semantic status tokens** (`--spec-status-pass/fail/warn/idle`, A1) — the **green/red/amber/idle** language for passing/failing/running/cancelled, applied consistently to the loop rail's per-node state, KPI tiles, console/run banner, suite/UC status cells, and Evidence rows, always paired with the text label (`[data-status]` colour-blind-safe contract preserved). **These are NOT the brand accent.**
- **The brand accent** (`--spec-accent`, teal — T2) is reserved for **chrome**: links, primary CTAs, focus rings, the active loop-rail *step* highlight, and active-navigation states — never a pass/fail status colour. (Keeping status and brand on separate tokens is what lets the run states stay distinguishable while the product still reads as Specorator.)
...
| R4 | **Re-run failed only** (§3.6) | H | M | **Med** — failure set must come from **this run** (capture the current run's failed Scenario Refs / a `runId`-filtered query — `latestStatuses()` is global and would re-run unrelated stale failures); plus a NEW failure-set `ExecutionScope` (or temp tag/grep) + runner-command/event/evidence changes | current-run results (not just global `latestStatuses`); **new** `ExecutionScope` + RunLauncher/runner-command threading |
...
| R8 | **Light identity system:** shared status-chip, Gherkin rhythm, consistent stage icons. **Two distinct token families** (Codex catch): run/automation **status** uses the semantic `--spec-status-pass/fail/warn/idle` (A1) so passed/failed/running/cancelled stay visually distinct under the `[data-status]` contract; the **brand `--spec-accent`** (teal, T2) is reserved for chrome / active-nav / the active loop-rail step — **never** the status colour (§3.7) | M | M | Low — builds on A1 tokens / A2 status-chip | A1 `--spec-status-*` (status) + `--spec-accent` (chrome); A2 `.spec-pill`; keep the `[data-status]` reinforcement contract |
...
6. ~~**Identity ceiling (R8)**~~ — **DECIDED (§0/T2): a single custom accent token (`--spec-accent`, the brand teal) + iconography is the ceiling.** R8's loop rail / status-chip / run-state accent draw `--spec-accent` (not stock-only Obsidian colours); status reinforcement stays colour-blind-safe via `[data-status]`. Not an open fork — point R8 at the brand token.
