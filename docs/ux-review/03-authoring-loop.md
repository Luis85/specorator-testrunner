...
- After "Generate," **open the step file at the inserted stub** automatically (the service already returns `stepFile` — `use-case-detail-rows.ts:219`). The user lands *on the code to write*, not at a count.
...
- Offer a **"Run against…" affordance** (modifier-click or a split-button caret on Run) that opens the existing `EnvironmentPickerModal` and runs once against the chosen env without changing the persisted active one. The plumbing exists (`EnvironmentPickerModal`, env injected at execute time); this just lets the choice happen at the point of intent.
...
- In the **feature editor**, show **"Included in N suites"** under each scenario's tag chips (reuse the same `scenarioCounter`/tag-matching used by `SuiteDashboardView` — `suite-dashboard-view.ts:104`), so authors see the consequence of a tag immediately.
...
- When a structured-editor scenario rename would drop history, **intercept on blur with an inline confirm** ("Scenario *Checkout* has 47 runs of history — renaming starts fresh. [Rename] [Keep name]"), weighting the decision with the *count* (history depth is already known — US-057). Keep the passive strip as a backstop, but make the destructive case a deliberate choice, not an after-the-fact footnote.
...
- **"Re-run failed only"** alongside Re-run (`test-console-view.ts:239`): the per-scenario history that powers the roll-up already knows which scenarios failed — scope a targeted re-run to them. Highest-value micro-interaction in the whole loop.
...
| R2 | **Pending Steps panel** + auto-open step file at generated stub; merge detect+generate into one "Steps" action (§3.2) | H | M | Med — needs a `.testrunner` step-file open path + line target | StepDefinitionService (returns `stepFile`); workspace open |
...
| R4 | **Re-run failed only** (§3.6) | H | M | Low — failed set already known from history | Scenario History (US-057); RunLauncher scope plumbing |
| R5 | **Run-time environment selector + env chip on console/launch** (§3.3) | H | M | Low — execute already reads active env; add per-run override | EnvironmentPickerModal; execute request shape |
| R6 | **Tag-expression builder + "in N suites" in editor + tag glossary** (§3.4) | M | M | Low — reuses listKnownTags + scenarioCounter | FeatureInsightService (tags, counter) |
| R7 | **Inline rename-identity confirm weighted by history depth** (§3.5) | M | M | Med — editor commit is currently fire-and-forget on blur | Scenario History; feature-editor commit path |
