...
| R8 | **Light identity system:** shared status-chip, run-state accent token (= `--spec-accent`, the decided brand teal — T2), Gherkin rhythm, consistent stage icons (§3.7) | M | M | Low — builds on A1 tokens / A2 status-chip | A1 `--spec-accent`; A2 `.spec-pill`; keep the `[data-status]` reinforcement contract |
