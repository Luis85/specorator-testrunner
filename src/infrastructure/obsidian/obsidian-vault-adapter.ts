...
export class ObsidianVaultAdapter implements VaultFileSystem {
  ...
  async exists(path: VaultPath): Promise<boolean> {
    return this.pathExists(normalizePath(path));
  }
  ...
  async readFile(path: VaultPath): Promise<Result<string>> {
    ...
    try {
      ...
      // Vault-API resolved null above; check adapter for unindexed paths
      // (`.testrunner/` files that Obsidian does not index).
      if (await this.app.vault.adapter.exists(normalized)) {
        // adapter API: unindexed file content is not in Obsidian's index
  ...
  async listFilesRecursive(path: VaultPath): Promise<Result<VaultPath[]>> {
    const normalized = normalizePath(path);
    try {
      if (!(await this.pathExists(normalized))) return ok([]);
      const files: VaultPath[] = [];
      const queue = [normalized];
      let dir: string | undefined;
      while ((dir = queue.shift()) !== undefined) {
        // adapter API: recursive directory listing is not in Obsidian's index
  ...
  async listFolders(): Promise<Result<VaultPath[]>> {
    ...
    try {
      ...
      let dir: string | undefined;
      while ((dir = queue.shift()) !== undefined) {
        // adapter API: full folder tree traversal is not in Obsidian's index
        const listing = await this.app.vault.adapter.list(dir);
  ...
  async deleteFolder(path: VaultPath): Promise<Result<void>> {
    ...
    try {
      ...
      if (!(await this.pathExists(normalized))) return ok(undefined);
      ...
      if (folder) {
        ...
        // adapter API: dot-folder removal is not in Obsidian's index
   ...
   * adapter is the deliberate fallback for paths Obsidian does not index
   * (`.testrunner/`, fresh evidence partitions). Used at every existence site
   * in this adapter to keep the pattern in one place.
   */
  private async pathExists(normalizedPath: string): Promise<boolean> {
    if (this.app.vault.getAbstractFileByPath(normalizedPath) !== null) return true;
