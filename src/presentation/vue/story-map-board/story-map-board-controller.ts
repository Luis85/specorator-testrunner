...
import { renderLoadError } from "../../views/modal-helpers";
import { RenderScheduler } from "../../views/render-scheduler";
import { buildBoardScene, type SvgNodeSpec } from "../../views/story-map-board-scene";
import { StoryMapCardModal } from "../../views/story-map-card-modal";
import {
  type BoardLayout,
  computeBoardLayout,
...
} from "../../views/story-map-board-layout";
import { makeDraggable } from "../../views/story-map-board-dnd";
import type { StoryMapBoardDeps } from "./story-map-board-deps";
 ...
 */
export class StoryMapBoardController {
  private storyMapId: string | null;
  ...
  // Coalesced, serialized reloads — the RenderScheduler the old LiveDashboardView
  // wrapped, owned directly so the board keeps its stale-write guard (ADR-0033).
  private readonly scheduler = new RenderScheduler(() => this.render());
  /** Set by the first close() so a second (onUnmounted) call is a no-op. */
  ...
  constructor(
    ...
    initialStoryMapId: string | null,
  ) {
    this.storyMapId = initialStoryMapId;
  }

  /**
   * Retargets the board to another map (the leaf was reused — the thin view's
   * setState wrote a new persisted id, which the mounted app watches). Mirrors the
   * old setState retarget.
   */
  // fallow-ignore-next-line complexity
  async setStoryMapId(next: string | null): Promise<void> {
    if (next === this.storyMapId) return;
    ...
    // target, dropping the rename. Same teardown-blur guard as close().
    ...
    this.model = null;
    this.baseline = "";
    if (this.isOpen) await this.scheduler.schedule();
  }

  /** Subscribes the origin-filtered map events and renders. Called once on mount. */
  async open(): Promise<void> {
    this.isOpen = true;
    // Manual subscription so we can inspect the payload's origin (the generic
    // refresh reloads blindly). External updates reload; our own saves are ignored.
  ...
  /**
   * Flushes the pending save, tears down DnD + subscriptions, disposes the
   * scheduler. Called from the app's onUnmounted (the thin view awaits it via the
   * unmount path so closing the leaf durably persists the last board edit).
   */
  // Structural cyclomatic count (idempotency guard + optional-chain teardowns) on
  // test-exempt view code; CRAP is a 0-coverage artifact.
  // fallow-ignore-next-line complexity
  async close(): Promise<void> {
    // Idempotent: the thin view awaits close() before unmounting (so a leaf-close
    // right after an edit persists it), and the app's onUnmounted calls it again.
  ...
  private wireDnd(svg: SVGSVGElement, layout: BoardLayout): void {
    this.cleanups.push(
      makeDraggable(this.host, ".sm-board-card-group", {
      ...
      makeDraggable(this.host, ".sm-board-activity", {
  ...
  private async runSaveLoop(): Promise<boolean> {
    while (this.dirty) {
      ...
      if (drifted && !this.dirty && this.model === model && this.commitEditor === null) {
        this.model = saved;
        if (this.isOpen) this.paint(this.host);
      }
    }
    return true;
  }
